...
function initPage() {
    page = 0;
    const releases = document.getElementById('blog-area');
...
function generateTitleArea(data) {
    ...
    subtitleArea.innerText = data.writer;
...
function generateMetaInfoArea(data) {
    ...
    metaInfoArea.innerText = data.date;
...
function addPage(page) {
    $.ajax({
        ...
        if (result.success) {
            ...
            if (post.length == 0 && page == 0) {
                $('#no-data').css('display', 'block');
                return;
            }

            let releasesArea = document.getElementById('blog-area');
            post.forEach(release => {
